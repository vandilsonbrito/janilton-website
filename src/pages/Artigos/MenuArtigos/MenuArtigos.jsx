...
export default function MenuArtigos() {
    ...
    const { titles, descriptions, textContents, imagesurl, slugs } = handleDocument();
    
    const showArticles = (titles, descriptions, textContents, imagesurl, slugs) => {
        const artigosDisplays = [];
        for (let i = 0; i < titles.length; i++) {
            artigosDisplays.push(<DisplayArtigos key={i} title={titles[i]} description={descriptions[i]} textContent={textContents[i]} imageurl={imagesurl[i]} slug={slugs[i]} />);
        }
        console.log('Menu de Artigos carregados!')
    ...
    return (
        <>
            ...
            <motion.div 
                ...
                {<HeaderOutter/>}
                <div className='w-full h-full bg-white text-white py-28 lg:py-40 flex flex-col justify-center'>
                    ...
                    <div className="w-full h-full flex flex-wrap justify-center items-center gap-8 xl:gap-14">
                        { (titles && descriptions && textContents && imagesurl && slugs) ? 
                            showArticles(titles, descriptions, textContents, imagesurl, slugs) 
                            :
                                <>
                                    <SkeletonArticle/>
                                    <SkeletonArticle/>
